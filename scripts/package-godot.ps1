...
function Write-WindowsCompilerHelp {
    ...
    Write-Host "    4. Run: powershell -NoProfile -ExecutionPolicy Bypass -File scripts\package-godot.ps1"
    Write-Host ""
    Write-Host "  Option B: MinGW-w64"
    Write-Host "    Run: powershell -NoProfile -ExecutionPolicy Bypass -File scripts\package-godot.ps1 -UseMinGW -MingwPrefix C:\msys64\mingw64"
    Write-Host ""
...
function Assert-WindowsCompiler {
    ...
    Write-WindowsCompilerHelp
    throw "Windows C++ compiler is required before building Godot."
}
...
function Get-ProductPattern {
    ...
    if ($Platform -eq "windows") {
        if ($MonoBuild) {
            return "^godot\.$platformPart\.$targetPart\.$archPart(\..*)?\.mono(\..*)?\.exe$"
        }
    ...
    if ($MonoBuild) {
        return "^godot\.$platformPart\.$targetPart\.$archPart(\..*)?\.mono(\..*)?$"
    }

    return "^godot\.$platformPart\.$targetPart\.$archPart(\..+)?$"
}
...
if (-not (Test-Path "SConstruct")) {
    throw "SConstruct was not found. Run this script from the Godot source tree."
}

$Version = Get-GodotVersion -VersionFile (Join-Path $RepoRoot "version.py")
$Timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
if ([string]::IsNullOrWhiteSpace($PackageName)) {
    $monoSuffix = if ($Mono) { "-mono" } else { "" }
    $PackageName = "godot-$Version-$Platform-$Target-$Arch$monoSuffix-$Timestamp"
}
...
if (-not $SkipBuild) {
    ...
    if ($detectedMsvcVersion -and -not (Test-HasSConsArg -Arguments $SConsArgs -Name "msvc_version")) {
        ...
        if ($detectedMsvcVersion -eq "14.5") {
            ...
            Write-Host "Godot requires SCons 4.10.1+ for Visual Studio 2026." -ForegroundColor Yellow
        }
    }
    ...
    Write-Step "Building Godot ($($buildArgs -join ' '))"
    ...
    if ($Mono) {
        ...
        $monoExecutable = Select-GodotExecutable -Products $monoProducts
        ...
        $ErrorActionPreference = "Continue"
        try {
            & $python ./modules/mono/build_scripts/build_assemblies.py --godot-output-dir=./bin --godot-platform=$Platform 2>&1 | Tee-Object -FilePath $assembliesLogPath
...
if ($Mono) {
    $godotSharpDir = Join-Path $BinDir "GodotSharp"
    if (-not (Test-Path $godotSharpDir)) {
        throw "Mono package requires '$godotSharpDir', but it was not found. Build assemblies may have failed or been skipped."
    }

    Copy-Item -Path $godotSharpDir -Destination $StagingDir -Recurse -Force
}
