"""
Load this file to your GDB session to enable pretty-printing of some Jundot C++ types.

GDB command: `source misc/utility/jundot_gdb_pretty_print.py`.
...
...
    {
        "description": "Load custom pretty-printers for Jundot types.",
        "text": "source ${workspaceFolder}/misc/utility/jundot_gdb_pretty_print.py"
    }
]
```
...
# Printer for Jundot StringName variables.
class JundotStringNamePrinter:
...
# Printer for Jundot String variables.
class JundotStringPrinter:
    def __init__(self, value):
        self.value = value
...
# Printer for Jundot Vector variables.
class JundotVectorPrinter:
    def __init__(self, value):
        self.value = value
...
def lookup_pretty_printer(value):
    if value.type.name == "StringName":
        return JundotStringNamePrinter(value)
    if value.type.name == "String":
        return JundotStringPrinter(value)
    if value.type.name and VECTOR_REGEX.match(value.type.name):
        return JundotVectorPrinter(value)
    return None
