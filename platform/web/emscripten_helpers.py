...
def create_engine_file(env, target, source, externs, threads_enabled):
    ...
    subst_dict = {"___JUNDOT_THREADS_ENABLED": "true" if threads_enabled else "false"}
...
def create_template_zip(env, js, wasm, side):
    binary_name = "jundot.editor" if env.editor_build else "jundot"
    ...
    if env["dlink_enabled"]:
        in_files.append(side)  # Side wasm (contains the actual Jundot code).
    ...
    if env.editor_build:
        ...
        cache = [
            ...
            "offline.html",
            "jundot.editor.js",
            "jundot.editor.audio.worklet.js",
            "jundot.editor.audio.position.worklet.js",
            "logo.svg",
            "favicon.png",
            "inter-regular.woff2",
            "inter-bold.woff2",
        ]
        opt_cache = ["jundot.editor.wasm"]
        subst_dict = {
            "___JUNDOT_VERSION___": get_build_version(False),
            "___JUNDOT_NAME___": "JundotEngine",
            "___JUNDOT_CACHE___": json.dumps(cache),
            "___JUNDOT_OPT_CACHE___": json.dumps(opt_cache),
            "___JUNDOT_OFFLINE_PAGE___": "offline.html",
            "___JUNDOT_THREADS_ENABLED___": "true" if env["threads"] else "false",
            "___JUNDOT_ENSURE_CROSSORIGIN_ISOLATION_HEADERS___": "true",
        }
        html = env.Substfile(target="#bin/jundot${PROGSUFFIX}.html", source=html, SUBST_DICT=subst_dict)
        ...
        service_worker = env.Substfile(
            target="#bin/jundot${PROGSUFFIX}.service.worker.js",
        ...
        out_files.append(zip_dir.File("jundot.offline.html"))
