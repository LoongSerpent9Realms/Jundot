...
def version_info_builder(target, source, env):
    with methods.generated_wrapper(str(target[0])) as file:
        file.write(
            """\
#define JUNDOT_VERSION_SHORT_NAME "{short_name}"
#define JUNDOT_VERSION_NAME "{name}"
#define JUNDOT_VERSION_MAJOR {major}
#define JUNDOT_VERSION_MINOR {minor}
#define JUNDOT_VERSION_PATCH {patch}
#define JUNDOT_VERSION_STATUS "{status}"
#define JUNDOT_VERSION_BUILD "{build}"
#define JUNDOT_VERSION_MODULE_CONFIG "{module_config}"
#define JUNDOT_VERSION_WEBSITE "{website}"
#define JUNDOT_VERSION_DOCS_BRANCH "{docs_branch}"
#define JUNDOT_VERSION_DOCS_URL "https://docs.jundotengine.org/en/" JUNDOT_VERSION_DOCS_BRANCH
...
const char *const JUNDOT_VERSION_HASH = "{git_hash}";
const unsigned long long JUNDOT_VERSION_TIMESTAMP = {git_timestamp};
""".format(**source[0].read())
        )
...
def make_license_header(target, source, env):
    ...
    with methods.generated_wrapper(str(target[0])) as file:
        file.write(f"""\
inline constexpr const char *JUNDOT_LICENSE_TEXT = {{
