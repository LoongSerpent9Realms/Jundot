...
Install one of these toolchains, then run this tool again:
  ...
  Option B: MinGW-w64
	...
	private async Task BuildAsync()
	{
		...
		if (_cfg.PlatformName == "android")
		{
			AddIfMissing(buildArgs, "generate_android_binaries", "yes");
		}
		...
		ThrowIfAndroidGradleFailed(buildLogPath);
	...
	private async Task PackageAsync()
	{
		...
		var binDir = Path.Combine(_repoRoot, "bin");
		var productDir = GetProductDirectory();
		if (!Directory.Exists(productDir))
			throw new Exception($"{productDir} directory not found. Build first or uncheck Skip Build.");

		var pattern = GetProductPattern(monoBuild: _cfg.Mono);
		var regex = new Regex(pattern, RegexOptions.IgnoreCase);
		var products = Directory.GetFiles(productDir)
			.Select(f => new FileInfo(f))
			.Where(f => regex.IsMatch(f.Name))
		...
		if (products.Count == 0)
			throw new Exception($"No build products matched '{pattern}' in {productDir}.");
	...
	private string GetProductPattern(bool monoBuild)
	{
		if (_cfg.PlatformName == "android")
		{
			if (string.Equals(_actualTarget, "editor", StringComparison.OrdinalIgnoreCase))
				return @"^android_editor-android-(?:debug|release).*?\.(?:apk|aab)$";

			var templateType = string.Equals(_actualTarget, "template_release", StringComparison.OrdinalIgnoreCase)
				? "release"
				: "debug";

			if (monoBuild)
				return templateType == "release" ? @"^android_monoRelease\.apk$" : @"^android_monoDebug\.apk$";

			return $"^android_{templateType}\\.apk$";
		}
	...
	{
		var binDir = Path.Combine(_repoRoot, "bin");
		if (_cfg.PlatformName == "android" && string.Equals(_actualTarget, "editor", StringComparison.OrdinalIgnoreCase))
			return Path.Combine(binDir, "android_editor_builds");
	...
	private void ThrowIfAndroidGradleFailed(string buildLogPath)
	{
		if (_cfg.PlatformName != "android" || !File.Exists(buildLogPath))
			return;

		var log = File.ReadAllText(buildLogPath);
		if (log.Contains("FAILURE: Build failed", StringComparison.OrdinalIgnoreCase) ||
			log.Contains("BUILD FAILED", StringComparison.OrdinalIgnoreCase))
		{
			throw new Exception($"Android Gradle build failed. See the real Gradle error above or in the build log: {buildLogPath}");
		}
	}
