...
String AIContextBuilder::_format_suggestion_instructions() {
	return "You are explicitly authorized to modify the engine's own source code (especially files in editor/ai/) to implement feature requests from the user. "
		   ...
		   "When you encounter a build error, read the build log to analyze the error, then use edit_file for localized fixes or write_file for complete file content, and run_build to verify.\n\n"
		   ...
		   "1. Call list_files('.', 2), search_files('editor/ai/'), or read_files to check if editor/ai/ exists\n"
		   ...
		   "   edit_file or write_file to apply changes, run_build to compile, check_build_status to wait for completion, "
		   "   and restart_engine to load the new build.\n"
		   "4. Workflow: list_files → search_files → read_files → edit_file/write_file → run_build → check_build_status → restart_engine\n\n"
		   "=== Tool Call Protocol (CRITICAL) ===\n"
		   "You MUST use the available tools (list_files, read_files, search_files, grep_code, edit_file, write_file, run_build, etc.) "
		   ...
		   "- Then make localized changes with edit_file, or use write_file when replacing the complete file\n"
