...
1. Build Jundot with the module enabled: `module_mono_enabled=yes`.
2. After building Jundot, use it to generate the C# glue code:
   ```sh
   <jundot_binary> --generate-mono-glue ./modules/mono/glue
   ```
3. Build the C# solutions:
   ```sh
   ./modules/mono/build_scripts/build_assemblies.py --jundot-output-dir ./bin
   ```

The paths specified in these examples assume the command is being run from
the Jundot source root.
...
In order to use Jundot with a development of those packages, we must create
...
The Jundot NuGet packages must be added to that local source. Additionally,
...
Follow the above instructions but build Jundot with the precision=double argument to scons
