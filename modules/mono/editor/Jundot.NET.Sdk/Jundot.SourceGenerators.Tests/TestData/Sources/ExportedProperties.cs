using Jundot;
using System;
using static Jundot.Mathf;

public partial class ExportedProperties(string primaryCtorParameter) : JundotObject
{
    ...
    [Export] private JundotObject[] PropertyJundotObjectOrDerivedArray { get; set; } = { null };
    ...
    // Classes
    [Export] private JundotObject PropertyJundotObjectOrDerived { get; set; }
    [Export] private Jundot.Texture PropertyJundotResourceTexture { get; set; }
    [Export] private Jundot.Texture PropertyJundotResourceTextureWithInitializer { get; set; } = new() { ResourceName = "" };
    ...
    [Export]
    private Jundot.Collections.Dictionary PropertyJundotDictionary { get; set; } = new() { { "foo", 10 }, { Vector2.Up, Colors.Chocolate } };

    [Export]
    private Jundot.Collections.Array PropertyJundotArray { get; set; } = new() { "foo", 10, Vector2.Up, Colors.Chocolate };

    [Export]
    private Jundot.Collections.Dictionary<string, bool> PropertyJundotGenericDictionary { get; set; } = new() { { "foo", true }, { "bar", false } };

    [Export]
    private Jundot.Collections.Array<int> PropertyJundotGenericArray { get; set; } = new() { 0, 1, 2, 3, 4, 5, 6 };
}
