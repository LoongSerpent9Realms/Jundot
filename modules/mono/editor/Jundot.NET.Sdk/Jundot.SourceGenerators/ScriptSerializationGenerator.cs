...
namespace Jundot.SourceGenerators
{
    ...
    public class ScriptSerializationGenerator : ISourceGenerator
    {
        ...
        {
            if (context.IsJundotSourceGeneratorDisabled("ScriptSerialization"))
                return;
        ...
        )
        {
            ...
            // TODO: We should still restore read-only properties after reloading assembly. Two possible ways: reflection or turn RestoreJundotObjectData into a constructor overload.
            // Ignore properties without a getter, without a setter or with an init-only setter. Jundot properties must be both readable and writable.
            var jundotClassProperties = propertySymbols.Where(property => !(property.IsReadOnly || property.IsWriteOnly || property.SetMethod!.IsInitOnly))
                .WhereIsJundotCompatibleType(typeCache)
                .ToArray();
            var jundotClassFields = fieldSymbols.Where(property => !property.IsReadOnly)
                .WhereIsJundotCompatibleType(typeCache)
                .ToArray();
            ...
            var signalDelegateSymbols = members
                ...
                .Where(s => s.GetAttributes()
                    .Any(a => a.AttributeClass?.IsJundotSignalAttribute() ?? false));

            List<JundotSignalDelegateData> jundotSignalDelegates = new();
            ...
            foreach (var signalDelegateSymbol in signalDelegateSymbols)
            {
                ...
                var invokeMethodData = signalDelegateSymbol
                    .DelegateInvokeMethod?.HasJundotCompatibleSignature(typeCache);

                if (invokeMethodData == null)
                    continue;

                jundotSignalDelegates.Add(new(signalName, signalDelegateSymbol, invokeMethodData.Value));
            }
            ...
            source.Append("        base.SaveJundotObjectData(info);\n");
            ...
            foreach (var signalDelegate in jundotSignalDelegates)
            {
            ...
            source.Append("        base.RestoreJundotObjectData(info);\n");
