...
namespace Jundot.SourceGenerators
{
    ...
    public class ScriptPropertyDefValGenerator : ISourceGenerator
    {
        ...
        {
            if (context.IsJundotSourceGeneratorDisabled("ScriptPropertyDefVal"))
                return;
        ...
        )
        {
            ...
            bool isNode = symbol.InheritsFrom("JundotSharp", JundotClasses.Node);
            ...
            var exportedProperties = members
                ...
                .Where(s => s.GetAttributes()
                    .Any(a => a.AttributeClass?.IsJundotExportAttribute() ?? false))
                .ToArray();
            ...
            foreach (var property in exportedProperties)
            {
                ...
                // Two possible ways: reflection or turn RestoreJundotObjectData into a constructor overload.
                // Ignore properties without a getter, without a setter or with an init-only setter.
                // Jundot properties must be both readable and writable.
            ...
            foreach (var field in exportedFields)
            {
                ...
                // Ignore properties without a getter or without a setter. Jundot properties must be both readable and writable.
                if (field.IsReadOnly)
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        Common.ExportedMemberIsReadOnlyRule,
            ...
            // Generate GetJundotExportedProperties
            ...
            if (exportedMembers.Count > 0)
            {
                ...
                const string DictionaryType =
                    "global::System.Collections.Generic.Dictionary<global::Jundot.StringName, global::Jundot.Variant>";

                source.Append("#if TOOLS\n");

                source.Append("    /// <summary>\n")
                    .Append("    /// Get the default values for all properties declared in this class.\n")
                    .Append("    /// This method is used by Jundot to determine the value that will be\n")
                ...
                source.Append(" GetJundotPropertyDefaultValues()\n    {\n");
        ...
        {
            if (marshalType == MarshalType.JundotObjectOrDerived)
            {
                return memberType.InheritsFrom("JundotSharp", JundotClasses.Node);
            }
            if (marshalType == MarshalType.JundotObjectOrDerivedArray)
            {
                var elementType = ((IArrayTypeSymbol)memberType).ElementType;
                return elementType.InheritsFrom("JundotSharp", JundotClasses.Node);
            }
            if (memberType is INamedTypeSymbol { IsGenericType: true } genericType)
            {
                return genericType.TypeArguments
                    .Any(static typeArgument
                        => typeArgument.InheritsFrom("JundotSharp", JundotClasses.Node));
            }
