...
namespace Jundot.SourceGenerators
{
    ...
    public class ScriptPropertiesGenerator : ISourceGenerator
    {
        ...
        {
            if (context.IsJundotSourceGeneratorDisabled("ScriptProperties"))
                return;
        ...
        )
        {
            ...
            bool isToolClass = symbol.GetAttributes().Any(a => a.AttributeClass?.IsJundotToolAttribute() ?? false);
            ...
            var jundotClassProperties = propertySymbols.WhereIsJundotCompatibleType(typeCache).ToArray();
            var jundotClassFields = fieldSymbols.WhereIsJundotCompatibleType(typeCache).ToArray();
            ...
            foreach (var property in jundotClassProperties)
            {
            ...
            foreach (var field in jundotClassFields)
            {
            ...
            if (jundotClassProperties.Length > 0 || jundotClassFields.Length > 0)
            {

                // Generate SetJundotClassPropertyValue

                bool allPropertiesAreReadOnly = jundotClassFields.All(fi => fi.FieldSymbol.IsReadOnly) &&
                                                jundotClassProperties.All(pi => pi.PropertySymbol.IsReadOnly || pi.PropertySymbol.SetMethod!.IsInitOnly);

                if (!allPropertiesAreReadOnly)
                {
                    source.Append("    /// <inheritdoc/>\n");
                    source.Append("    [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]\n");
                    source.Append("    protected override bool SetJundotClassPropertyValue(in jundot_string_name name, ");
                    source.Append("in jundot_variant value)\n    {\n");
                    ...
                    source.Append("        return base.SetJundotClassPropertyValue(name, value);\n");

                    source.Append("    }\n");
                }

                // Generate GetJundotClassPropertyValue
                bool allPropertiesAreWriteOnly = jundotClassFields.Length == 0 && jundotClassProperties.All(pi => pi.PropertySymbol.IsWriteOnly);

                if (!allPropertiesAreWriteOnly)
                {
                    source.Append("    /// <inheritdoc/>\n");
                    source.Append("    [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]\n");
                    source.Append("    protected override bool GetJundotClassPropertyValue(in jundot_string_name name, ");
                    source.Append("out jundot_variant value)\n    {\n");
                    ...
                    source.Append("        return base.GetJundotClassPropertyValue(name, out value);\n");

                    source.Append("    }\n");
                }
                // Generate GetJundotPropertyList

                const string DictionaryType = "global::System.Collections.Generic.List<global::Jundot.Bridge.PropertyInfo>";

                source.Append("    /// <summary>\n")
                    .Append("    /// Get the property information for all the properties declared in this class.\n")
                    .Append("    /// This method is used by Jundot to register the available properties in the editor.\n")
                ...
                var jundotClassPropertiesAndFields = Enumerable.Empty<JundotPropertyOrFieldData>()
                    .Concat(jundotClassProperties.Select(propertyData => new JundotPropertyOrFieldData(propertyData)))
                    .Concat(jundotClassFields.Select(fieldData => new JundotPropertyOrFieldData(fieldData)))
                    .OrderBy(data => data.Symbol.Locations[0].Path())
                    .ThenBy(data => data.Symbol.Locations[0].StartLine());

                foreach (var member in jundotClassPropertiesAndFields)
                {
        ...
        {
            source.Append("        properties.Add(new(type: (global::Jundot.Variant.Type)")
        ...
        private static IEnumerable<PropertyInfo> DetermineGroupingPropertyInfo(ISymbol memberSymbol)
        {
            foreach (var attr in memberSymbol.GetAttributes())
            {
                ...
                {
                    JundotClasses.ExportCategoryAttr => PropertyUsageFlags.Category,
                    JundotClasses.ExportGroupAttr => PropertyUsageFlags.Group,
                    JundotClasses.ExportSubgroupAttr => PropertyUsageFlags.Subgroup,
                    _ => null
                };
        ...
        )
        {
            var exportAttr = memberSymbol.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.IsJundotExportAttribute() ?? false);

            var exportToolButtonAttr = memberSymbol.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.IsJundotExportToolButtonAttribute() ?? false);
                ...
                static bool ExpressionBodyReturnsNewCallable(Compilation compilation, ArrowExpressionClauseSyntax? expressionSyntax)
                {
                    ...
                    switch (expressionSyntax.Expression)
                    {
                        ...
                        case ObjectCreationExpressionSyntax creationExpression:
                            ...
                            if (typeSymbol != null)
                            {
                                return typeSymbol.FullQualifiedNameOmitGlobal() == JundotClasses.Callable;
                            }
                            break;
                        ...
                        case InvocationExpressionSyntax invocationExpression:
                            ...
                            {
                                return methodSymbol.ContainingType.FullQualifiedNameOmitGlobal() == JundotClasses.Callable;
                            }
                            break;
                    }
        ...
        )
        {
            ...
            if (variantType == VariantType.Object && type is INamedTypeSymbol memberNamedType)
            {
                ...
                if (memberNamedType.InheritsFrom("JundotSharp", "Jundot.Resource"))
                {
                ...
                if (memberNamedType.InheritsFrom("JundotSharp", "Jundot.Node"))
                {
            ...
            static string GetTypeName(INamedTypeSymbol memberSymbol)
            {
                if (memberSymbol.GetAttributes()
                    .Any(a => a.AttributeClass?.IsJundotGlobalClassAttribute() ?? false))
                {
                    return memberSymbol.Name;
                }

                return memberSymbol.GetJundotScriptNativeClassName()!;
            }
            ...
            if (!isTypeArgument && variantType == VariantType.Dictionary)
            {
                ...
                {
                    // To maintain compatibility with previous versions of Jundot before 4.4,
