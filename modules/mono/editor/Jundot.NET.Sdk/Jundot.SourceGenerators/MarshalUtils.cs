...
namespace Jundot.SourceGenerators
{
    internal static class MarshalUtils
    {
        ...
        {
            public INamedTypeSymbol JundotObjectType { get; }
            ...
            public TypeCache(Compilation compilation)
            {
                ...
                JundotObjectType = GetTypeByMetadataNameOrThrow(JundotClasses.JundotObject);
            }
        }
        ...
        public static VariantType? ConvertMarshalTypeToVariantType(MarshalType marshalType)
            => marshalType switch
            {
                ...
                MarshalType.JundotObjectOrDerivedArray => VariantType.Array,
                ...
                MarshalType.JundotObjectOrDerived => VariantType.Object,
                MarshalType.StringName => VariantType.StringName,
                MarshalType.NodePath => VariantType.NodePath,
                MarshalType.Rid => VariantType.Rid,
                MarshalType.JundotDictionary => VariantType.Dictionary,
                MarshalType.JundotArray => VariantType.Array,
                MarshalType.JundotGenericDictionary => VariantType.Dictionary,
                MarshalType.JundotGenericArray => VariantType.Array,
                _ => null
            };
        ...
        public static MarshalType? ConvertManagedTypeToMarshalType(ITypeSymbol type, TypeCache typeCache)
        {
            ...
            switch (specialType)
            {
                ...
                default:
                {
                    ...
                    {
                        if (type.ContainingAssembly?.Name == "JundotSharp" &&
                            type.ContainingNamespace?.Name == "Jundot")
                        {
                            return type switch
                            {
                    ...
                    else if (typeKind == TypeKind.Array)
                    {
                        ...
                        if (elementType.SimpleDerivesFrom(typeCache.JundotObjectType))
                            return MarshalType.JundotObjectOrDerivedArray;

                        if (elementType.ContainingAssembly?.Name == "JundotSharp" &&
                            elementType.ContainingNamespace?.Name == "Jundot")
                        {
                            switch (elementType)
                            {
                                case { Name: "Vector2" }:
                    ...
                    {
                        if (type.SimpleDerivesFrom(typeCache.JundotObjectType))
                            return MarshalType.JundotObjectOrDerived;
                        ...
                        if (type.ContainingAssembly?.Name == "JundotSharp")
                        {
                            switch (type.ContainingNamespace?.Name)
                            {
                                ...
                                case "Collections"
                                    when type.ContainingNamespace?.FullQualifiedNameOmitGlobal() == "Jundot.Collections":
                                    return type switch
                                    {
                                        { Name: "Dictionary" } =>
                                            type is INamedTypeSymbol { IsGenericType: false } ?
                                                MarshalType.JundotDictionary :
                                                MarshalType.JundotGenericDictionary,
                                        { Name: "Array" } =>
                                            type is INamedTypeSymbol { IsGenericType: false } ?
                                                MarshalType.JundotArray :
                                                MarshalType.JundotGenericArray,
                                        _ => null
                                    };
                            }
                        }
        ...
        private const string VariantUtils = "global::Jundot.NativeInterop.VariantUtils";
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            ...
            {
                // We need a special case for JundotObjectOrDerived[], because it's not supported by VariantUtils.ConvertTo<T>
                MarshalType.JundotObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".ConvertToSystemArrayOfJundotObject<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">(",
                        inputExpr, ")"),
                // We need a special case for generic Jundot collections and JundotObjectOrDerived[], because VariantUtils.ConvertTo<T> is slower
                MarshalType.JundotGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for JundotObjectOrDerived[], because it's not supported by VariantUtils.CreateFrom<T>
                MarshalType.JundotObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".CreateFromSystemArrayOfJundotObject(", inputExpr, ")"),
                // We need a special case for generic Jundot collections and JundotObjectOrDerived[], because VariantUtils.CreateFrom<T> is slower
                MarshalType.JundotGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for JundotObjectOrDerived[], because it's not supported by Variant.As<T>
                MarshalType.JundotObjectOrDerivedArray =>
                    source.Append(inputExpr, ".AsJundotObjectArray<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">()"),
                // We need a special case for generic Jundot collections and JundotObjectOrDerived[], because Variant.As<T> is slower
                MarshalType.JundotGenericDictionary =>
                    source.Append(inputExpr, ".AsJundotDictionary<",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[0].FullQualifiedNameIncludeGlobal(), ", ",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[1].FullQualifiedNameIncludeGlobal(), ">()"),
                MarshalType.JundotGenericArray =>
                    source.Append(inputExpr, ".AsJundotArray<",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[0].FullQualifiedNameIncludeGlobal(), ">()"),
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for JundotObjectOrDerived[], because it's not supported by Variant.From<T>
                MarshalType.JundotObjectOrDerivedArray =>
                    source.Append("global::Jundot.Variant.CreateFrom(", inputExpr, ")"),
                // We need a special case for generic Jundot collections, because Variant.From<T> is slower
                MarshalType.JundotGenericDictionary or MarshalType.JundotGenericArray =>
                    source.Append("global::Jundot.Variant.CreateFrom(", inputExpr, ")"),
                _ => source.Append("global::Jundot.Variant.From<",
                    typeSymbol.FullQualifiedNameIncludeGlobal(), ">(", inputExpr, ")")
            };
        }
    }
