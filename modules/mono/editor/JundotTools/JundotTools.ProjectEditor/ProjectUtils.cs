...
namespace JundotTools.ProjectEditor
{
    ...
    {
        [GeneratedRegex(@"\s*'\$\(JundotTargetPlatform\)'\s*==\s*'(?<platform>[A-z]+)'\s*", RegexOptions.IgnoreCase)]
        private static partial Regex JundotTargetPlatformConditionRegex();
        ...
        public static void UpgradeProjectIfNeeded(MSBuildProject project, string projectName)
        {
            ...
            EnsureJundotSdkIsUpToDate(project);
        ...
        public static void EnsureJundotSdkIsUpToDate(MSBuildProject project)
        {
            var root = project.Root;
            string jundotSdkAttrValue = ProjectGenerator.JundotSdkAttrValue;

            if (!string.IsNullOrEmpty(root.Sdk) &&
                root.Sdk.Trim().Equals(jundotSdkAttrValue, StringComparison.OrdinalIgnoreCase))
                return;

            root.Sdk = jundotSdkAttrValue;
            project.HasUnsavedChanges = true;
        }
        ...
        {
            var root = project.Root;
            string minTfmValue = ProjectGenerator.JundotMinimumRequiredTfm;
            ...
            foreach (var propertyGroup in root.PropertyGroups)
            {
                ...
                if (groupHasCondition && !ConditionMatchesJundotPlatform(propertyGroup.Condition))
                {
                    continue;
                }
                ...
                foreach (var property in propertyGroup.Properties)
                {
                    ...
                    if (propertyHasCondition && !ConditionMatchesJundotPlatform(property.Condition))
                    {
                        continue;
                    }
            ...
            foreach (var property in propertiesToChange)
            {
                ...
                // the minimum required by Jundot, we don't want to remove the conditional
            ...
            static bool ConditionMatchesJundotPlatform(string condition)
            {
                // Check if the condition is checking the 'JundotTargetPlatform' for one of the
                // Jundot platforms with built-in support in the Jundot.NET.Sdk.
                var match = JundotTargetPlatformConditionRegex().Match(condition);
                if (match.Success)
                {
