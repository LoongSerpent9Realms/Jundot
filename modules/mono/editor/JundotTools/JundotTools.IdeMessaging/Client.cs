...
using JundotTools.IdeMessaging.Utils;
...
namespace JundotTools.IdeMessaging
{
    ...
    public sealed class Client : IDisposable
    {
        ...
        private JundotIdeMetadata jundotIdeMetadata;
        private readonly FileSystemWatcher fsWatcher;

        public string JundotEditorExecutablePath => jundotIdeMetadata.EditorExecutablePath;
        ...
        public Client(string identity, string jundotProjectDir, IMessageHandler messageHandler, ILogger logger)
        {
            this.identity = identity;
            this.messageHandler = messageHandler;
            this.logger = logger;

            string projectMetadataDir = Path.Combine(jundotProjectDir, ".jundot", "mono", "metadata");
            // FileSystemWatcher requires an existing directory
            if (!Directory.Exists(projectMetadataDir))
            {
                // Check if the non hidden version exists
                string nonHiddenProjectMetadataDir = Path.Combine(jundotProjectDir, "jundot", "mono", "metadata");
            ...
            MetaFilePath = Path.Combine(projectMetadataDir, JundotIdeMetadata.DefaultFileName);

            fsWatcher = new FileSystemWatcher(projectMetadataDir, JundotIdeMetadata.DefaultFileName);
        }
        ...
        private async void OnMetaFileChanged(object sender, FileSystemEventArgs e)
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (metadata != null && metadata != jundotIdeMetadata)
                {
                    jundotIdeMetadata = metadata.Value;
        ...
        private JundotIdeMetadata? ReadMetadataFile()
        {
            ...
            using (var reader = new StreamReader(fileStream))
            {
                ...
                return new JundotIdeMetadata(port, editorExecutablePath);
            }
        }
        ...
        private async Task ConnectToServer()
        {
            ...
            try
            {
                logger.LogInfo("Connecting to Jundot Ide Server");

                await tcpClient.ConnectAsync(IPAddress.Loopback, jundotIdeMetadata.Port);

                logger.LogInfo("Connection open with Jundot Ide Server");
            ...
            catch (SocketException e)
            {
                if (e.SocketErrorCode == SocketError.ConnectionRefused)
                    logger.LogError("The connection to the Jundot Ide Server was refused");
                else
                    throw;
            }
        }
        ...
        public async void Start()
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (!File.Exists(MetaFilePath))
                {
                    logger.LogInfo("There is no Jundot Ide Server running");
                    return;
                }
                ...
                {
                    logger.LogError("Failed to read Jundot Ide metadata file");
                }
            }
        }
            ...
            where TResponse : Response, new()
        {
            if (!IsConnected)
            {
                logger.LogError("Cannot write request. Not connected to the Jundot Ide Server.");
                return null;
            }
