...
namespace JundotTools.IdeMessaging.CLI
{
    internal static class Program
    {
        ...
        private static async Task<int> StartAsync(string[] args, Stream inputStream, Stream outputStream)
        {
            ...
            try
            {
                ...
                string jundotProjectDir = args[0];

                if (!Directory.Exists(jundotProjectDir))
                {
                    Logger.LogError($"The specified Jundot project directory does not exist: {jundotProjectDir}");
                    return 1;
                }

                var forwarder = new ForwarderMessageHandler(outputWriter);

                using (var fwdClient = new Client("VisualStudioCode", jundotProjectDir, forwarder, Logger))
                {
                    fwdClient.Start();
