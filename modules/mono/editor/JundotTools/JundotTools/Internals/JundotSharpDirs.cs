...
{
    public static class JundotSharpDirs
    {
        public static string ResMetadataDir
        {
            get
            {
                Internal.jundot_icall_JundotSharpDirs_ResMetadataDir(out jundot_string dest);
                using (dest)
        ...
        public static string MonoUserDir
        {
            get
            {
                Internal.jundot_icall_JundotSharpDirs_MonoUserDir(out jundot_string dest);
                using (dest)
                    return Marshaling.ConvertStringToManaged(dest);
            }
        }
        ...
        public static string BuildLogsDirs
        {
            get
            {
                Internal.jundot_icall_JundotSharpDirs_BuildLogsDirs(out jundot_string dest);
                using (dest)
                    return Marshaling.ConvertStringToManaged(dest);
            }
        }
        ...
        public static string DataEditorToolsDir
        {
            get
            {
                Internal.jundot_icall_JundotSharpDirs_DataEditorToolsDir(out jundot_string dest);
                using (dest)
                    return Marshaling.ConvertStringToManaged(dest);
            }
        }
        ...
        public static string CSharpProjectName
        {
            get
            {
                Internal.jundot_icall_JundotSharpDirs_CSharpProjectName(out jundot_string dest);
                using (dest)
                    return Marshaling.ConvertStringToManaged(dest);
            }
        }
        ...
        public static void DetermineProjectLocation()
        {
            ...
            // The csproj should be in the same folder as project.jundot.
        ...
        private static string FindSolutionFileWithAssemblyName(string directory, string assemblyName)
        {
            ...
            foreach (string solutionFilePath in solutionFilePaths)
            {
                ...
                {
                    // Convert '\' path separators on Windows to '/' to match Jundot's Unix style separators
        ...
        public static string ProjectBaseOutputPath
        {
            get
            {
                if (_projectCsProjPath == null)
                    DetermineProjectLocation();
                return Path.Combine(Path.GetDirectoryName(_projectCsProjPath)!, ".jundot", "mono", "temp", "bin");
            }
        }
