...
namespace JundotTools.Build
{
    public static class BuildSystem
    {
        ...
        {
            var verbosityLevel =
                editorSettings.GetSetting(JundotSharpEditor.Settings.VerbosityLevel).As<VerbosityLevelId>();
            arguments.Add("-v");
            ...
            if ((bool)editorSettings.GetSetting(JundotSharpEditor.Settings.NoConsoleLogging))
                arguments.Add("-noconlog");
        }

        private static void AddLoggerArgument(BuildInfo buildInfo, Collection<string> arguments)
        {
            string buildLoggerPath = Path.Combine(Internals.JundotSharpDirs.DataEditorToolsDir,
                "JundotTools.BuildLogger.dll");

            arguments.Add(
                $"-l:{typeof(JundotBuildLogger).FullName},{buildLoggerPath};{buildInfo.LogsDirPath}");
        }

        private static void AddBinaryLogArgument(BuildInfo buildInfo, Collection<string> arguments,
            EditorSettings editorSettings)
        {
            if (!(bool)editorSettings.GetSetting(JundotSharpEditor.Settings.CreateBinaryLog))
                return;
        ...
        {
            var baseDylib = $"{JundotSharpDirs.ProjectAssemblyName}.dylib";
            var baseSym = $"{JundotSharpDirs.ProjectAssemblyName}.framework.dSYM";
