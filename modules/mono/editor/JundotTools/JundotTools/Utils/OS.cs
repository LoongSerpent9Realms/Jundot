...
namespace JundotTools.Utils
{
    ...
    public static class OS
    {
        ...
        public static readonly Dictionary<string, string> DotNetOSPlatformMap = new Dictionary<string, string>
        {
            ...
            // Jundot has a single export profile for both, named LinuxBSD.
        ...
        {
            Internal.jundot_icall_Utils_OS_GetPlatformName(out jundot_string dest);
            using (dest)
            {
        ...
        private static string? PathWhichUnix(string name)
        {
            ...
            return searchDirs.Select(dir => Path.Combine(dir, name))
                .FirstOrDefault(path =>
                {
                    using jundot_string pathIn = Marshaling.ConvertStringToNative(path);
                    return File.Exists(path) && Internal.jundot_icall_Utils_OS_UnixFileHasExecutableAccess(pathIn);
                });
        }
