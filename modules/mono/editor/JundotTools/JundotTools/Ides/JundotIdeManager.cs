...
{
    public sealed partial class JundotIdeManager : Node, ISerializationListener
    {
        ...
        private MessagingServer GetRunningOrNewServer()
        {
            ...
            _messagingServer = new MessagingServer(OS.GetExecutablePath(),
                ProjectSettings.GlobalizePath(JundotSharpDirs.ResMetadataDir), new JundotLogger());
        ...
        public async Task<EditorPick?> LaunchIdeAsync(int millisecondsTimeout = 10000)
        {
            ...
            var editorId = editorSettings.GetSetting(JundotSharpEditor.Settings.ExternalEditor).As<ExternalEditorId>();
            return await LaunchIdeAsync(editorId, millisecondsTimeout);
        }

        public async Task<EditorPick?> LaunchIdeAsync(ExternalEditorId editorId, int millisecondsTimeout = 10000)
        {
        ...
        private void LaunchIde(ExternalEditorId editorId, string editorIdentity)
        {
            switch (editorId)
            {
                ...
                case ExternalEditorId.MonoDevelop:
                {
                    ...
                    try
                    {
                        var instance = GetMonoDevelopInstance(JundotSharpDirs.ProjectSlnPath);
        ...
        public readonly struct EditorPick
        {
            ...
            public bool IsAnyConnected() =>
                JundotSharpEditor.Instance.JundotIdeManager.GetRunningOrNewServer().IsAnyConnected(_identity);
                ...
                where TResponse : Response, new()
            {
                ...
                JundotSharpEditor.Instance.JundotIdeManager
        ...
        private class JundotLogger : ILogger
        {
