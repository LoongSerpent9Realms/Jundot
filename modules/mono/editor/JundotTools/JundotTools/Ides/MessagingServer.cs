...
using JundotTools.IdeMessaging;
...
namespace JundotTools.Ides
{
    public sealed class MessagingServer : IDisposable
    {
        ...
        public MessagingServer(string editorExecutablePath, string projectMetadataDir, ILogger logger)
        {
            ...
            _metaFilePath = Path.Combine(projectMetadataDir, JundotIdeMetadata.DefaultFileName);

            // Make sure the directory exists
            Directory.CreateDirectory(projectMetadataDir);

            // The Jundot editor's file system thread can keep the file open for writing, so we are forced to allow write sharing...
            ...
            where TResponse : Response, new()
        {
            using (await _peersSem.UseAsync())
            {
                ...
                {
                    _logger.LogError("Cannot write request. No client connected to the Jundot Ide Server.");
                    return;
                }
        ...
        private class ServerMessageHandler : IMessageHandler
        {
            private static void DispatchToMainThread(Action action)
            {
                ...
                Jundot.Dispatcher.SynchronizationContext.Post(d, null);
            }
            ...
            private static Task<Response> HandleDebugPlay(DebugPlayRequest request)
            {
                DispatchToMainThread(() =>
                {
                    ...
                    JundotSharpEditor.Instance.SkipBuildBeforePlaying = !(request.BuildBeforePlaying ?? true);
                    ...
                    Environment.SetEnvironmentVariable("JUNDOT_MONO_DEBUGGER_AGENT",
                    ...
                    Environment.SetEnvironmentVariable("JUNDOT_MONO_DEBUGGER_AGENT", "");
                    JundotSharpEditor.Instance.SkipBuildBeforePlaying = false;
                });
            ...
            private static async Task<Response> HandleCodeCompletionRequest(CodeCompletionRequest request)
            {
                ...
                await Jundot.Engine.GetMainLoop().ToSignal(Jundot.Engine.GetMainLoop(), "process_frame");
