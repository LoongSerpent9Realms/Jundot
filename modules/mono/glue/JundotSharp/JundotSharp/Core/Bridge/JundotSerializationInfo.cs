...
public sealed class JundotSerializationInfo : IDisposable
{
    ...
    private JundotSerializationInfo(in jundot_dictionary properties, in jundot_dictionary signalEvents)
    {
    ...
    internal static JundotSerializationInfo CreateCopyingBorrowed(
        in jundot_dictionary properties, in jundot_dictionary signalEvents)
    {
        return new(NativeFuncs.jundotsharp_dictionary_new_copy(properties),
            NativeFuncs.jundotsharp_dictionary_new_copy(signalEvents));
    }
        ...
        where T : Delegate
    {
        ...
        {
            if (DelegateUtils.TryDeserializeDelegate(serializedData.AsJundotArray(), out var eventDelegate))
            {
