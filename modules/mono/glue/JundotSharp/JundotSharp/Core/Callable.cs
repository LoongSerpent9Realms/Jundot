...
namespace Jundot
{
    ...
    public readonly partial struct Callable
    {
        ...
        private readonly unsafe delegate* managed<object, NativeVariantPtrArgs, out jundot_variant, void> _trampoline;

        /// <summary>
        /// Object that contains the method.
        /// </summary>
        public JundotObject Target => _target;
        ...
        /// </summary>
        public unsafe delegate* managed<object, NativeVariantPtrArgs, out jundot_variant, void> Trampoline
            => _trampoline;
        ...
        public unsafe Callable(JundotObject target, StringName method)
        {
            _target = target;
            _method = method;
            _delegate = null;
        ...
        {
            _target = @delegate?.Target as JundotObject;
            _method = null;
            _delegate = @delegate;
        ...
        {
            using jundot_callable callable = Marshaling.ConvertCallableToNative(this);

            int argc = args.Length;

            Span<jundot_variant.movable> argsStoreSpan = argc <= VarArgsSpanThreshold ?
                stackalloc jundot_variant.movable[VarArgsSpanThreshold] :
                new jundot_variant.movable[argc];

            Span<IntPtr> argsSpan = argc <= VarArgsSpanThreshold ?
                stackalloc IntPtr[VarArgsSpanThreshold] :
                new IntPtr[argc];

            fixed (jundot_variant* varargs = &MemoryMarshal.GetReference(argsStoreSpan).DangerousSelfRef)
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                for (int i = 0; i < argc; i++)
                {
                    varargs[i] = (jundot_variant)args[i].NativeVar;
                    argsPtr[i] = new IntPtr(&varargs[i]);
                }

                jundot_variant ret = NativeFuncs.jundotsharp_callable_call(callable,
                    (jundot_variant**)argsPtr, argc, out jundot_variant_call_error vcall_error);
                ExceptionUtils.DebugCheckCallError(callable, (jundot_variant**)argsPtr, argc, vcall_error);
        ...
        public unsafe void CallDeferred(scoped ReadOnlySpan<Variant> args)
        {
            ...
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                ...
                NativeFuncs.jundotsharp_callable_call_deferred(callable, (jundot_variant**)argsPtr, argc);
            }
        }
        ...
        ///    <item>
        ///        <term>args</term>
        ///        <description>Array of <see cref="jundot_variant"/> arguments.</description>
        ///    </item>
        ///    <item>
        ///        <term>ret</term>
        ///        <description>Return value of type <see cref="jundot_variant"/>.</description>
        ///    </item>
        ///</list>
        /// <para>
        ...
        ///
        /// <code>
        ///     static void Trampoline(object delegateObj, NativeVariantPtrArgs args, out jundot_variant ret)
        ///     {
        ///         if (args.Count != 1)
