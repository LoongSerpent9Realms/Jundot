...
public partial class VariantUtils
{
    ...
    {
        internal delegate jundot_variant ToVariantConverter(scoped in T from);
        internal delegate T FromVariantConverter(in jundot_variant from);

        public static unsafe jundot_variant ToVariant(scoped in T from) =>
             ToVariantCb != null ? ToVariantCb(from) : throw UnsupportedType<T>();

        public static unsafe T FromVariant(in jundot_variant variant) =>
    ...
    public static jundot_variant CreateFrom<[MustBeVariant] T>(scoped in T from)
    {
        ...
        if (typeof(T) == typeof(Jundot.Collections.Dictionary))
            return CreateFromDictionary(UnsafeAs<Jundot.Collections.Dictionary>(from));

        if (typeof(T) == typeof(Jundot.Collections.Array))
            return CreateFromArray(UnsafeAs<Jundot.Collections.Array>(from));

        if (typeof(T) == typeof(Variant))
            return NativeFuncs.jundotsharp_variant_new_copy((jundot_variant)UnsafeAs<Variant>(from).NativeVar);
        ...
        if (typeof(JundotObject).IsAssignableFrom(typeof(T)))
            return CreateFromJundotObject(UnsafeAs<JundotObject>(from));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static T ConvertTo<[MustBeVariant] T>(in jundot_variant variant)
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        if (typeof(JundotObject).IsAssignableFrom(typeof(T)))
            return (T)(object)ConvertToJundotObject(variant);
