...
namespace Jundot.NativeInterop
{
    internal static class ExceptionUtils
    {
        ...
        private static void SendToScriptDebugger(Exception e)
        {
            ...
            using jundot_string nFile = Marshaling.ConvertStringToNative(file);
            using jundot_string nFunc = Marshaling.ConvertStringToNative(func);
            using jundot_string nErrorMsg = Marshaling.ConvertStringToNative(errorMsg);
            using jundot_string nExcMsg = Marshaling.ConvertStringToNative(excMsg.ToString());

            using DebuggingUtils.jundot_stack_info_vector stackInfoVector = default;
            ...
            unsafe
            {
                ...
                {
                    DebuggingUtils.jundot_stack_info* stackInfo = &stackInfoVector.Elements[i];
                ...
                NativeFuncs.jundotsharp_internal_script_debugger_send_error(nFunc, nFile, line,
                    nErrorMsg, nExcMsg, jundot_error_handler_type.ERR_HANDLER_ERROR, stackInfoVector);
            }
        }
        ...
        public static void LogException(Exception e)
        {
            try
            {
                if (NativeFuncs.jundotsharp_internal_script_debugger_is_active().ToBool())
                {
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(jundot_string_name method, IntPtr instance, jundot_variant** args, int argCount, jundot_variant_call_error error)
        {
            if (error.Error != jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_OK)
            {
                using jundot_variant instanceVariant = VariantUtils.CreateFromJundotObjectPtr(instance);
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(in jundot_callable callable, jundot_variant** args, int argCount, jundot_variant_call_error error)
        {
            if (error.Error != jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_OK)
            {
                using jundot_variant callableVariant = VariantUtils.CreateFromCallableTakingOwnershipOfDisposableValue(callable);
        ...
        private unsafe static string GetCallErrorWhere(ref jundot_variant_call_error error, jundot_string_name method, jundot_variant* instance, jundot_variant** args, int argCount)
        {
            string? methodstr = null;
            string basestr = GetVariantTypeName(instance);

            if (method == JundotObject.MethodName.Call || (basestr == "Jundot.TreeItem" && method == TreeItem.MethodName.CallRecursive))
            {
                if (argCount >= 1)
                {
                    methodstr = VariantUtils.ConvertToString(*args[0]);
                    if (error.Error == jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT)
                    {
                        error.Argument += 1;
                    }
                }
        ...
        private unsafe static string GetCallErrorMessage(jundot_variant_call_error error, string where, jundot_variant** args)
        {
            switch (error.Error)
            {
                case jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT:
                {
                ...
                case jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_ERROR_TOO_MANY_ARGUMENTS:
                case jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_ERROR_TOO_FEW_ARGUMENTS:
                    return $"Invalid call to {where}. Expected {error.Expected} argument(s).";
                case jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_ERROR_INVALID_METHOD:
                    return $"Invalid call. Nonexistent {where}.";
                case jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_ERROR_INSTANCE_IS_NULL:
                    return $"Attempt to call {where} on a null instance.";
                case jundot_variant_call_error_error.JUNDOT_CALL_ERROR_CALL_ERROR_METHOD_NOT_CONST:
        ...
        private unsafe static string GetVariantTypeName(jundot_variant* variant)
        {
            if (variant->Type == Variant.Type.Object)
            {
                JundotObject obj = VariantUtils.ConvertToJundotObject(*variant);
                if (obj == null)
                {
                    return "null instance";
                }
                else if (!JundotObject.IsInstanceValid(obj))
                {
